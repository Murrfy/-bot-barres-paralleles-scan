...
export function planAutomaticTargetExit({
  ...
  if (candidates.length === 1) {
    ...
    const structurallyValid = String(existing?.timeInForce || '').toUpperCase() === 'GTC' &&
      n(existing?.price) > 0 &&
      n(existing?.origQty) > 0 &&
      n(existing?.executedQty, 0) >= 0 &&
      n(existing?.origQty) - n(existing?.executedQty, 0) > 0;
  ...
  if (existing) {
    const remaining = n(existing?.origQty) - n(existing?.executedQty, 0);
    const quantityMatches = Math.abs(remaining - live.quantity) <= Math.max(1e-9, live.quantity * 1e-10);
    const priceMatches = Math.abs(n(existing?.price) - desired.targetPrice) <= Math.max(1e-9, Math.abs(desired.targetPrice) * 1e-10);
    if (quantityMatches && priceMatches) {
      return { action:'NONE', reason:'MANAGED_TARGET_ALREADY_OPEN', live, existing, ...desired };
    }
    return {
      action:'REPLACE',
      reason:'MANAGED_TARGET_REFRESH_REQUIRED',
      live,
      existing,
      previousClientOrderId:String(existing.clientOrderId || ''),
      ...desired,
    };
  }
  ...
  return {
    action:'PLACE',
    reason:exactEnabled?'EXACT_SALE_TARGET_REQUIRED':'CALCULATED_TARGET_REQUIRED',
    live,
    ...desired,
  };
}

...
function createCycle({symbol,direction,time,orderId,clientOrderId,qty,price,commission,commissionAsset}){
  const commissions={};
  signedAdd(commissions,commissionAsset,commission);
  return {
    symbol,
    direction,
    openedAt:time,
...
function addOpen(cycle,{qty,price,commission,commissionAsset}){
  ...
  signedAdd(cycle.commissions,commissionAsset,commission);
}
function finalizeCycle(cycle,fundingRows){
  ...
  const commissionUsdt=n(cycle.commissions.USDT,0);
  const fundingUsdt=n(fundingByAsset.USDT,0);
  const nonUsdtCommission=Object.entries(cycle.commissions)
  ...
  return {
    ...
    exactNetUsdt,
    commissionByAsset:cycle.commissions,
    fundingByAsset,
...
export function buildZenithClosedTradeHistory({trades=[],orders=[],funding=[]}={}){
  ...
  for(const trade of sorted){
    ...
    if(state.cycle){
      ...
      signedAddScaled(state.cycle.commissions,trade.commissionAsset,trade.commission,ratio);

...
function feeAdd(target,asset,amount){
  const key=upper(asset)||'UNKNOWN';
  const value=Math.abs(n(amount,0));
  if(!(value>0))return;
  target[key]=n(target[key],0)+value;
}
function feeAddScaled(target,asset,amount,ratio){
  const r=Math.max(0,Math.min(1,n(ratio,0)));
  feeAdd(target,asset,n(amount,0)*r);
}
...
function createCycle({symbol,direction,time,orderId,clientOrderId,qty,price,commission,commissionAsset}){
  const fees={};
  feeAdd(fees,commissionAsset,commission);
  return {
    symbol,
    direction,
    openedAt:time,
...
function addOpen(cycle,{qty,price,commission,commissionAsset}){
  ...
  feeAdd(cycle.fees,commissionAsset,commission);
}
function finalizeCycle(cycle,fundingRows){
  ...
  for(const row of fundingRows){
    ...
    const type=upper(row?.incomeType);
    if(type!=='FUNDING_FEE')continue;
    const asset=upper(row?.asset)||'UNKNOWN';
    fundingByAsset[asset]=n(fundingByAsset[asset],0)+n(row?.income,0);
  }
  ...
  const nonUsdtFees=Object.entries(cycle.fees).filter(([asset,amount])=>asset!=='USDT'&&n(amount)>0);
  const nonUsdtFunding=Object.entries(fundingByAsset).filter(([asset,amount])=>asset!=='USDT'&&Math.abs(n(amount))>0);
  const exactNetUsdt=nonUsdtFees.length===0&&nonUsdtFunding.length===0;
  return {
    version:1,
...
export function buildZenithClosedTradeHistory({trades=[],orders=[],funding=[]}={}){
  ...
  const sorted=(Array.isArray(trades)?trades:[])
    .map(row=>({
      ...
      commission:Math.abs(n(row?.commission,0)),
  ...
  for(const trade of sorted){
    ...
    let state=states.get(trade.symbol)||{qty:0,cycle:null};
    ...
    if(state.cycle){
      ...
      feeAddScaled(state.cycle.fees,trade.commissionAsset,trade.commission,ratio);
    ...
    if(crossed&&state.cycle){
      ...
      if(state.cycle.zenithOwned)closed.push(finalizeCycle(state.cycle,Array.isArray(funding)?funding:[]));
      state.cycle=null;
    }
...
export function mergeTradeHistory(existing=[],fresh=[],limit=500){
  ...
  return [...byId.values()].sort((a,b)=>n(b.closedAt)-n(a.closedAt)).slice(0,Math.max(1,Math.min(1000,n(limit,500))));
}

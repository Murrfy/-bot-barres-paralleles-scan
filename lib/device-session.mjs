...
export function roleAssignmentKey(prefix, role) {
  const normalizedRole = String(role || '').toLowerCase();
  if (!['controller', 'master'].includes(normalizedRole)) {
    throw new Error('DEVICE_ROLE_INVALID');
  }
  return `${String(prefix || '')}:role-issued-at:${normalizedRole}`;
}

export function deviceRoleAssignmentActive(record, issuedAtRaw) {
  const issuedAt = Number(issuedAtRaw || 0);
  if (!Number.isFinite(issuedAt) || issuedAt <= 0) return true;
  const createdAt = Number(record?.createdAt || 0);
  return Number.isFinite(createdAt) && createdAt >= issuedAt;
}
...
export function engineInstanceHeader(req) {
  return header(req, 'x-zenith-engine-instance').trim();
}

export function validEngineInstanceId(value) {
  return /^engine-instance-[A-Za-z0-9._:-]{16,96}$/.test(String(value || ''));
}

export function enginePrincipalInstanceActive(record, suppliedInstance, currentInstance) {
  if (String(record?.principal || '') !== 'engine') return true;
  const expected = String(record?.engineInstanceId || '');
  const supplied = String(suppliedInstance || '');
  const current = String(currentInstance || '');
  return validEngineInstanceId(expected) &&
    supplied === expected &&
    current === expected;
}
...
export function deviceTokenCandidates(req, { allowBearer = false } = {}) {
  const candidates = [cookieToken(req)];
  if (allowBearer) candidates.unshift(bearerToken(req));
...
export function expectedRequestOrigin(req) {
  ...
  const host = header(req, 'host').split(',')[0].trim();

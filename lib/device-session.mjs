...
export function serverMasterLeaseKey(prefix) {
  return `${String(prefix || '')}:server-master-instance`;
}

export function serverMasterInstanceActive(record, leaseRaw) {
  if (String(record?.deviceKind || '') !== 'server-master') return true;
  const instanceId = String(record?.workerInstanceId || '');
  return Boolean(instanceId) && String(leaseRaw || '') === instanceId;
}

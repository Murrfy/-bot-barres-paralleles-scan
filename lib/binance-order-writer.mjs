...
export async function cancelEntryOrderIdempotent({
  ...
  if (!/^[A-Z0-9]{3,30}$/.test(sym) || !/^zth-ENT-[a-f0-9]{24}$/i.test(cid)) {
    throw new Error('CANCEL_TARGET_NOT_ZENITH_ENTRY');
  }
  ...
  if (String(existing?.clientOrderId || '') !== cid) {
    throw new BinanceRequestError('CANCEL_TARGET_CLIENT_ID_MISMATCH', { data: existing });
  }
  if (String(existing?.side || '').toUpperCase() !== 'BUY') {
    throw new BinanceRequestError('CANCEL_TARGET_NOT_BUY', { data: existing });
  }
  if (String(existing?.type || '').toUpperCase() !== 'LIMIT') {
    throw new BinanceRequestError('CANCEL_TARGET_NOT_LIMIT', { data: existing });
  }
  if (String(existing?.timeInForce || '').toUpperCase() !== 'GTC') {
    throw new BinanceRequestError('CANCEL_TARGET_NOT_GTC', { data: existing });
  }

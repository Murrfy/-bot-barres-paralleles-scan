...
function nearlyEqualNumber(a, b) {
  ...
  return Math.abs(aa - bb) <= Math.max(1e-12, Math.abs(bb) * 1e-10);
}

function standardLimitMatches(order, { side, quantity, price } = {}) {
  return Boolean(
    order &&
    ...
    nearlyEqualNumber(order.origQty, quantity) &&
    nearlyEqualNumber(order.price, price)
  );
}

export async function modifyStandardLimitOrderIdempotent({
  fetchImpl = fetch,
  apiKey,
  secret,
  symbol,
  ...
  if (!/^[A-Z0-9]{3,30}$/.test(sym) || !cid || cid.length > 36 ||
      !['BUY','SELL'].includes(normalizedSide) || !(qty > 0) || !(px > 0)) {
    throw new Error('MODIFY_ORDER_FIELDS_INVALID');
  }
  ...
  try {
    ...
    if (orderNotFound(error)) {
      throw new BinanceRequestError('MODIFY_TARGET_UNKNOWN', {
        code: error.code,
        data: { symbol: sym, clientOrderId: cid },
        ambiguous: true,
      });
  ...
  const status = String(existing?.status || '').toUpperCase();
  if (terminalOrderStatus(status)) {
    throw new BinanceRequestError('MODIFY_TARGET_TERMINAL', { data: existing });
  }
  if (String(existing?.type || '').toUpperCase() !== 'LIMIT' ||
      String(existing?.timeInForce || 'GTC').toUpperCase() !== 'GTC' ||
      String(existing?.positionSide || 'BOTH').toUpperCase() !== 'BOTH' ||
      !(existing?.reduceOnly === true || existing?.reduceOnly === 'true') ||
      String(existing?.side || '').toUpperCase() !== normalizedSide) {
    throw new BinanceRequestError('MODIFY_TARGET_NOT_REDUCE_ONLY_LIMIT', { data: existing });
  }
  if (Number(existing?.executedQty || 0) > 1e-12) {
    throw new BinanceRequestError('MODIFY_TARGET_PARTIALLY_FILLED', { data: existing });
  }
  if (standardLimitMatches(existing, { side: normalizedSide, quantity: qty, price: px })) {
    return {
      ok: true,
  ...
  try {
    const modified = await signedBinanceRequest({
      fetchImpl,
      path: '/fapi/v1/order',
      method: 'PUT',
      apiKey,
    ...
    if (String(modified?.clientOrderId || '') !== cid ||
        !standardLimitMatches(modified, { side: normalizedSide, quantity: qty, price: px })) {
      throw new BinanceRequestError('ORDER_MODIFY_RESPONSE_MISMATCH', {
        data: modified,
        ambiguous: true,
      });
    }
    ...
    try {
      ...
      if (standardLimitMatches(after, { side: normalizedSide, quantity: qty, price: px })) {
        return {
          ok: true,
          disposition: 'RECOVERED_AFTER_AMBIGUOUS_MODIFY',
          writeAttempted: true,
          order: after,
        };
      }
      if (terminalOrderStatus(after?.status)) {
        throw new BinanceRequestError('ORDER_MODIFY_TARGET_TERMINAL', {
          data: after,
          ambiguous: true,
        });
      }
      if (error.ambiguous) {
        throw new BinanceRequestError('ORDER_MODIFY_RESULT_AMBIGUOUS', {
          data: {
            symbol: sym,
            clientOrderId: cid,
            observedPrice: after?.price,
            observedQty: after?.origQty,
      ...
      if (queryError instanceof BinanceRequestError &&
          ['ORDER_MODIFY_TARGET_TERMINAL','ORDER_MODIFY_RESULT_AMBIGUOUS'].includes(queryError.message)) {
        throw queryError;
      }
      if (orderNotFound(queryError)) {
        throw new BinanceRequestError('ORDER_MODIFY_RESULT_AMBIGUOUS', {
          data: { symbol: sym, clientOrderId: cid, originalError: error.message },
          ambiguous: true,
        });
      }
      throw queryError;

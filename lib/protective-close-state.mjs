...
export function closeTargetRemaining(beforeQuantity, requestedQuantity) {
  const before = Math.max(0, n(beforeQuantity));
  const requested = Math.max(0, n(requestedQuantity));
  if (!(before > 0) || !(requested > 0) || requested > before + 1e-12) {
    throw new Error('CLOSE_QUANTITY_INVALID');
  }
  return Math.max(0, before - requested);
}

export function evaluateProtectiveClose({
  state,
  symbol,
  direction,
  beforeQuantity,
  ...
  const before = Math.max(0, n(beforeQuantity));
  const targetRemaining = closeTargetRemaining(before, requestedQuantity);
  ...
  const confirmed = afterQuantity <= targetRemaining + epsilon;
  ...
  return {
    ...
    requestedQuantity: Math.max(0, n(requestedQuantity)),
    targetRemaining,
    afterQuantity,

...
export function buildControllerCancelEntryCommand(order) {
  ...
  if (String(order?.side || '').toUpperCase() !== 'BUY') throw new Error('CANCEL_TARGET_NOT_BUY');
  if (String(order?.type || '').toUpperCase() !== 'LIMIT') throw new Error('CANCEL_TARGET_NOT_LIMIT');
  if (String(order?.timeInForce || '').toUpperCase() !== 'GTC') throw new Error('CANCEL_TARGET_NOT_GTC');

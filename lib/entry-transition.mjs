...
export function evaluateEntryTransitionReconciliation({
  ...
  const expiredProtectionOnly=[];
  const prunableExpired=[];
  for(const row of expired){
    ...
    if(live)continue;
    const exactEntry=transition.state==='ENTRY_SUBMITTED'
      ?orders.find(order=>transitionEntryMatches(order,transition))
      :null;
    const protection=orders.find(order=>transitionProtectionMatches(order,transition));
    if(exactEntry){
      active.push({...transition,expiredPendingEntry:true});
      continue;
    }
    if(protection){
      expiredProtectionOnly.push(transition);
      continue;
    }
    prunableExpired.push(transition);
  }

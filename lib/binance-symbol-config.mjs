...
async function verifyAfterWrite(args,code){
  try{return await readSymbolConfig(args)}
  catch(e){
    throw configFailure(code,{cause:e,writeAttempted:true,ambiguous:true});
  }
}

function autoAddMarginOff(config){
  return config?.isAutoAddMargin===false||String(config?.isAutoAddMargin||'').toLowerCase()==='false';
}
...
export async function ensureBinanceEntrySymbolConfig({
  ...
  if(!autoAddMarginOff(current)){
    const enabled=current?.isAutoAddMargin===true||String(current?.isAutoAddMargin||'').toLowerCase()==='true';
    throw configFailure(
      enabled?'BINANCE_AUTO_ADD_MARGIN_ENABLED':'BINANCE_AUTO_ADD_MARGIN_UNKNOWN',
  ...
  if(upper(current.marginType)!=='ISOLATED'){
    ...
    current=await verifyAfterWrite({
      requestImpl,apiKey,secret,symbol:wanted,timestamp,offset:offset++
    },'BINANCE_MARGIN_TYPE_VERIFY_UNAVAILABLE');
    if(upper(current.marginType)!=='ISOLATED'){
      throw configFailure(
  ...
  if(num(current.leverage)!==wantedLeverage){
    ...
    current=await verifyAfterWrite({
      requestImpl,apiKey,secret,symbol:wanted,timestamp,offset:offset++
    },'BINANCE_LEVERAGE_VERIFY_UNAVAILABLE');
    if(num(current.leverage)!==wantedLeverage){
      throw configFailure(

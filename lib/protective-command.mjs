...
export function protectionOnlyMismatchTarget(report){
  ...
  if(!Array.isArray(report.reasons)||report.reasons.length!==1||report.reasons[0]!=='MISSING_BINANCE_PROTECTION')return '';
  const missing=Array.isArray(report?.differences?.missingProtections)
    ?report.differences.missingProtections.map(x=>String(x||'').toUpperCase()).filter(Boolean)
    :[];

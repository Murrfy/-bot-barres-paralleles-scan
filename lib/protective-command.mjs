...
export function pendingEntryWriteAheadRecoveryTargets(report){
  ...
  for(const row of rows){
    ...
    const expiresAt=n(row.expiresAt);
    const canonicalProtection=isLimitIocMaxLossOrder(row,{
      symbol,
      side:direction==='LONG'?'SELL':'BUY',
      positionSide:'BOTH',
      quantity,
      clientAlgoId:protectionClientAlgoId,
    });
    if(!/^[A-Za-z0-9._:-]{8,128}$/.test(commandId)||
       !/^[A-Z0-9]{3,30}$/.test(symbol)||
       ...
       !canonicalProtection||!(triggerPrice>0)||
...
export function orphanZenithCleanupOrders(report){
  ...
  for(const row of rows){
    ...
    out.push({
      ...
      priceMatch:String(row.priceMatch||''),
    });
  }
  return out;
}

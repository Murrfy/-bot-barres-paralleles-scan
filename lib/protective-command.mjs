...
export function expiredEntryOrphanProtections(report){
  if(!report||report.version!==2||report.status!=='MISMATCH'||report.failClosed!==true)return [];
  const reasons=Array.isArray(report.reasons)?report.reasons.map(value=>String(value||'')):[];
  if(reasons.length!==1||reasons[0]!=='ORPHAN_ZENITH_PROTECTIVE_ORDER')return [];
  ...
  for(const row of expired){
    ...
    if(!/^[A-Za-z0-9._:-]{8,128}$/.test(commandId)||
       ...
       !['LONG','SHORT'].includes(direction)||
       !/^zth-MAX-[A-Za-z0-9._:-]+$/.test(protectionClientAlgoId)||protectionClientAlgoId.length>36||
       (entryClientOrderId&&(!/^zth-[A-Za-z0-9._:-]+$/.test(entryClientOrderId)||entryClientOrderId.length>36))){
      return [];
    }
    const exact=orphans.find(order=>
      String(order?.orderClass||'').toUpperCase()==='ALGO'&&
      String(order?.symbol||'').toUpperCase()===symbol&&
      String(order?.clientAlgoId||'')===protectionClientAlgoId&&
      String(order?.side||'').toUpperCase()===(direction==='LONG'?'SELL':'BUY')&&
    ...
    if(exact)out.push({commandId,symbol,direction,protectionClientAlgoId,entryClientOrderId});
  }
  return out;
}

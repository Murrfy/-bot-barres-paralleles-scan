...
function verifiedRiskSnapshot(riskSnapshot, symbol, now = Date.now()) {
  ...
  const referencePrice = positive(normalized.referencePrice, 'REFERENCE_PRICE_INVALID');
  return { normalized, quantity, leverage, referencePrice, observedAt, ageMs };
}
...
export function buildEntryOrderPlan({ command, riskSnapshot, now = Date.now() }) {
  ...
  if (type === 'LIMIT') {
    ...
    if (Math.abs(limitPrice - risk.referencePrice) > 1e-12) throw new Error('ENTRY_PREFLIGHT_PRICE_MISMATCH');
    params.price = String(risk.referencePrice);
  }

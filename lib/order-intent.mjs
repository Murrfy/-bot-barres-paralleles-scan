...
export function buildEntryOrderPlan({ command, riskSnapshot, now = Date.now() }) {
  ...
  const limitPrice = type === 'LIMIT' ? positive(command?.limitPrice, 'LIMIT_PRICE_INVALID') : 0;
  ...
  if (type === 'LIMIT' &&
      Math.abs(limitPrice - observedPrice) > Math.max(1e-9, Math.abs(observedPrice) * 1e-10)) {

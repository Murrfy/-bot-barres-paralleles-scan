...
export function buildEntryOrderPlan({ command, riskSnapshot, now = Date.now() }) {
  ...
  if (type !== 'LIMIT') throw new Error('ENTRY_ORDER_TYPE_LIMIT_REQUIRED');

  const risk = verifiedRiskSnapshot(riskSnapshot, symbol, now);
  const limitPrice = positive(command?.limitPrice, 'LIMIT_PRICE_INVALID');
  const expected = risk.normalized;
  const pairs = [
    ['margin', command?.margin, expected.margin, 'ENTRY_PREFLIGHT_MARGIN_MISMATCH'],
    ['leverage', command?.leverage, expected.leverage, 'ENTRY_PREFLIGHT_LEVERAGE_MISMATCH'],
    ['maxLoss', command?.maxLoss, expected.maxLoss, 'ENTRY_PREFLIGHT_MAX_LOSS_MISMATCH'],
  ];
  for (const [, requested, observed, code] of pairs) {
    const a = positive(requested, code);
    const b = positive(observed, code);
    if (Math.abs(a - b) > Math.max(1e-9, Math.abs(b) * 1e-10)) throw new Error(code);
  }
  const observedPrice = positive(expected.referencePrice, 'ENTRY_PREFLIGHT_PRICE_INVALID');
  if (Math.abs(limitPrice - observedPrice) > Math.max(1e-9, Math.abs(observedPrice) * 1e-10)) {
    throw new Error('ENTRY_PREFLIGHT_PRICE_MISMATCH');
  }
  ...
  params.price = String(limitPrice);
...
export function buildExitOrderPlan({
  ...
  if (mode === 'NORMAL_LIMIT') {
    ...
    if (!['OPPONENT','OPPONENT_5'].includes(match)) {

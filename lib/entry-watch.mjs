...
export function entryWatchSlotRaceOutcome({
  previousPendingUntil=0,
  crossingAt=0,
  now=Date.now(),
}={}){
  const at=Math.max(0,Math.floor(num(now,Date.now())));
  const inherited=Math.max(0,Math.floor(num(previousPendingUntil,0)));
  const crossed=Math.max(0,Math.floor(num(crossingAt,at)));
  const deadline=inherited>0?inherited:crossed+50000;
  if(at>=deadline){
    return {expired:true,pendingUntil:0,blockedAt:at,deadline};
  }
  return {expired:false,pendingUntil:deadline,blockedAt:0,deadline};
}

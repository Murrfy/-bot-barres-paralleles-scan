import { normalizeProtectiveUpdatePayload, protectionOnlyMismatchTarget, missingMaxLossRepairTarget, orphanZenithCleanupOrders } from './protective-command.mjs';
export { protectionOnlyMismatchTarget, missingMaxLossRepairTarget, orphanZenithCleanupOrders };
const CLOSE_MODES = new Set(['PROTECTIVE_IOC']);

function number(value) {
  const x = Number(value);
  return Number.isFinite(x) ? x : NaN;
}

function cleanSymbol(value) {
  const symbol = String(value || '').trim().toUpperCase();
  if (!/^[A-Z0-9]{3,30}$/.test(symbol)) throw new Error('SYMBOL_INVALID');
  return symbol;
}

export function masterExecutionEligible(state = {}) {
  const mode = String(state.mode || '').toUpperCase();
  return Boolean(
    state.role === 'master' &&
    state.hidden !== true &&
    state.leaseActive === true &&
    state.realExecutionArmed === true &&
    state.userStreamReady === true &&
    (mode === 'RUNNING' || mode === 'PAUSE_PENDING')
  );
}

export function buildMasterCommandDispatch(command) {
  if (!command || typeof command !== 'object') throw new Error('COMMAND_INVALID');
  const type = String(command.type || '').toUpperCase();
  if (!['EXEC_CLOSE_POSITION','EXEC_CANCEL_ENTRY','EXEC_UPDATE_EXIT','EXEC_UPDATE_PROTECTION','EXEC_UPDATE_ACTIVE_CONFIG','EXEC_OPEN_MARKET_POSITION'].includes(type)) {
    return { supported:false, reason:'MASTER_COMMAND_NOT_IMPLEMENTED', type };
  }

  const payload = command.payload && typeof command.payload === 'object' ? command.payload : {};

  if (type === 'EXEC_OPEN_MARKET_POSITION') {
    const symbol = cleanSymbol(payload.symbol);
    const side = String(payload.side || '').toUpperCase();
    const orderType = String(payload.orderType || '').toUpperCase();
    const margin = number(payload.margin);
    const leverage = number(payload.leverage);
    const maxLoss = number(payload.maxLoss);
    const requestedAt = number(payload.requestedAt);
    if (side !== 'BUY') throw new Error('MARKET_ENTRY_BUY_ONLY');
    if (orderType !== 'MARKET') throw new Error('MARKET_ENTRY_TYPE_REQUIRED');
    if (!(margin > 0)) throw new Error('MARGIN_INVALID');
    if (!(leverage > 0)) throw new Error('LEVERAGE_INVALID');
    if (!(maxLoss > 0)) throw new Error('MAX_LOSS_INVALID');
    if (!(requestedAt > 0)) throw new Error('REQUESTED_AT_INVALID');
    return {
      supported:true,
      type,
      endpoint:'/api/binance-entry-execute',
      body:{
        type,
        phase:'SUBMIT_MARKET_ENTRY',
        commandId:String(command.id || ''),
        symbol,
        side,
        orderType,
        margin,
        leverage,
        maxLoss,
        requestedAt,
      },
    };
  }

  if (type === 'EXEC_UPDATE_EXIT' || type === 'EXEC_UPDATE_PROTECTION') {
    const normalized = normalizeProtectiveUpdatePayload(type,payload);
    return {
      supported:true,
      type,
      endpoint:'/api/binance-protective-update-execute',
      body:{
        type,
        commandId:String(command.id || ''),
        ...normalized,
        ...(payload.activeConfig && typeof payload.activeConfig === 'object' && !Array.isArray(payload.activeConfig)
          ? {activeConfig:payload.activeConfig}
          : {}),
      },
    };
  }

  if (type === 'EXEC_UPDATE_ACTIVE_CONFIG') {
    const symbol = cleanSymbol(payload.symbol);
    const direction = String(payload.direction || '').toUpperCase();
    const quantity = number(payload.quantity);
    const activeConfig = payload.activeConfig;
    if (!['LONG','SHORT'].includes(direction)) throw new Error('DIRECTION_INVALID');
    if (!(quantity > 0)) throw new Error('QUANTITY_INVALID');
    if (!activeConfig || typeof activeConfig !== 'object' || Array.isArray(activeConfig)) {
      throw new Error('ACTIVE_CONFIG_REQUIRED');
    }
    return {
      supported:true,
      type,
      endpoint:'',
      body:{
        type,
        commandId:String(command.id || ''),
        symbol,
        direction,
        quantity,
        activeConfig,
      },
    };
  }

  const symbol = cleanSymbol(payload.symbol);

  if (type === 'EXEC_CANCEL_ENTRY') {
    const clientOrderId = String(payload.clientOrderId || '');
    if (!/^zth-ENT-[a-f0-9]{24}$/i.test(clientOrderId)) throw new Error('CANCEL_TARGET_NOT_ZENITH_ENTRY');
    return {
      supported:true,
      type,
      endpoint:'/api/binance-protective-execute',
      body:{
        type,
        commandId:String(command.id || ''),
        symbol,
        clientOrderId,
      },
    };
  }

  const direction = String(payload.direction || '').toUpperCase();
  if (!['LONG','SHORT'].includes(direction)) throw new Error('DIRECTION_INVALID');

  const quantity = number(payload.quantity);
  if (!(quantity > 0)) throw new Error('QUANTITY_INVALID');
  if (payload.closeAll !== true) throw new Error('CLOSE_ALL_REQUIRED');

  const exitMode = String(payload.exitMode || 'PROTECTIVE_IOC').toUpperCase();
  if (!CLOSE_MODES.has(exitMode)) throw new Error('EXIT_MODE_INVALID');

  const attemptRaw = Number(payload.attempt || 0);
  const attempt = Math.max(0, Math.min(20, Number.isInteger(attemptRaw) ? attemptRaw : 0));

  return {
    supported:true,
    type,
    endpoint:'/api/binance-protective-execute',
    body:{
      type,
      commandId:String(command.id || ''),
      symbol,
      direction,
      quantity,
      closeAll:true,
      exitMode,
      attempt,
    },
  };
}

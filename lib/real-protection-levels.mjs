...
export function exactPriceForLinearPnl({
  entryPrice,
  quantity,
  direction,
  pnlUsd,
  priceFilter,
  errorCode = 'PNL_NOT_TICK_REPRESENTABLE',
} = {}) {
  const filter = normalizePriceFilter(priceFilter);
  const rawPrice = priceForLinearPnl({ entryPrice, quantity, direction, pnlUsd });
  enforcePriceBounds(rawPrice, filter);
  if (!alignedToTick(rawPrice, filter.tickSize)) {
    const candidates = nearestTickCandidates(rawPrice, filter.tickSize);
    const e = new Error(errorCode);
    e.requestedPnlUsd = n(pnlUsd);
    e.rawPrice = rawPrice;
    e.tickSize = filter.tickSize;
    e.lowerPrice = candidates.lower;
    e.upperPrice = candidates.upper;
    e.lowerPnlUsd = pnlAtLinearPrice({ entryPrice, quantity, direction, price: candidates.lower });
    e.upperPnlUsd = pnlAtLinearPrice({ entryPrice, quantity, direction, price: candidates.upper });
    throw e;
  }
  return Number(rawPrice.toFixed(decimals(filter.tickSize)));
}

export function buildExactProgressiveProtectionLevel({
  position,
  protectedProfitUsd,
  priceFilter,
} = {}) {
  ...
  const protectedPrice = exactPriceForLinearPnl({
    entryPrice: live.entryPrice,
    quantity: live.quantity,
    direction: live.direction,
    pnlUsd: protectedPnl,
    priceFilter,
    errorCode: 'PROTECTED_PNL_NOT_TICK_REPRESENTABLE',
  });
  return {
    version: 1,
    symbol: live.symbol,
...
export function buildRealProtectionLevels({
  ...
  const targetPrice = exactPriceForLinearPnl({
    entryPrice: live.entryPrice,
    quantity: live.quantity,
    direction: live.direction,
    pnlUsd: target,
    priceFilter: filter,
    errorCode: 'TARGET_PNL_NOT_TICK_REPRESENTABLE',
  });

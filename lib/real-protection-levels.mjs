...
export function buildEmergencyMaxLossLevel({
  ...
  if (live.direction === 'LONG' && !(triggerPrice < live.entryPrice)) {
    throw new Error('LONG_MAX_LOSS_NOT_BELOW_ENTRY');
  }
  if (live.direction === 'SHORT' && !(triggerPrice > live.entryPrice)) {
    throw new Error('SHORT_MAX_LOSS_NOT_ABOVE_ENTRY');
  }

...
function algoKey(order) {
  const symbol = String(order?.s ?? order?.symbol ?? '').toUpperCase();
  const algoId = String(order?.aid ?? order?.algoId ?? '');
  const clientAlgoId = String(order?.caid ?? order?.clientAlgoId ?? order?.ca ?? order?.c ?? '');
...
export function markUserStreamConnected(inputState, { connectionId, at = Date.now() } = {}) {
  ...
  state.failReasons = ['RECONCILIATION_REQUIRED_AFTER_CONNECT'];
  state.failClosed = true;
  return state;
}
...
export function markUserStreamNeedsReconciliation(inputState, reason = 'RECONCILIATION_REQUIRED') {
  const state = clone(inputState || createUserStreamState());
  state.needsReconciliation = true;
  withReason(state, String(reason || 'RECONCILIATION_REQUIRED'));
  return state;
}
...
export function markUserStreamReconciled(inputState, { observedAt, runtimeHash } = {}) {
  ...
  state.failReasons = [];
  state.failClosed = state.connected !== true;
  return state;
}
...
export function applyUserDataEvent(inputState, event) {
  ...
  if (type === 'ALGO_UPDATE') {
    ...
    const normalized = {
      ...
      algoId: String(o.aid ?? o.algoId ?? ''),
      clientAlgoId: String(o.caid ?? o.clientAlgoId ?? o.ca ?? o.c ?? ''),
      ...
      triggerPrice: String(o.tp ?? o.sp ?? o.triggerPrice ?? ''),
      price: String(o.p ?? o.price ?? ''),
      reduceOnly: o.R === true || o.reduceOnly === true,
      closePosition: o.cp === true || o.closePosition === true,
      timeInForce: String(o.f ?? o.timeInForce ?? ''),
      workingType: String(o.wt ?? o.workingType ?? ''),
      priceMatch: String(o.pm ?? o.priceMatch ?? ''),
      quantity: String(o.q ?? o.quantity ?? ''),
      actualOrderId: String(o.ai ?? o.actualOrderId ?? ''),
      averagePrice: String(o.ap ?? o.averagePrice ?? ''),
      executedQuantity: String(o.aq ?? o.executedQty ?? ''),
      actualOrderType: String(o.act ?? o.actualOrderType ?? ''),
      triggerTime: num(o.tt ?? o.triggerTime),
      goodTillDate: num(o.gtd ?? o.goodTillDate),
      failedReason: String(o.rm ?? o.failedReason ?? ''),
      activated: o.ia === true,
      eventTime,
      transactionTime: num(event.T),
      raw: o,
  ...
  if (type === 'ACCOUNT_UPDATE') {
    ...
    for (const p of Array.isArray(a.P) ? a.P : []) {
      ...
      const previous=nextPositions[key]||null;
      const positionAmount=String(p.pa ?? '');
      const entryPrice=String(p.ep ?? '');
      const sameCore=Boolean(
        previous &&
        String(previous.positionAmount??'')===positionAmount &&
        String(previous.entryPrice??'')===entryPrice
      );
      const positionLifecycleAt=sameCore
        ?num(previous.positionLifecycleAt,num(previous.transactionTime,eventTime))
        :num(event.T,eventTime);
      nextPositions[key] = {
        ...
        isAutoAddMargin:previous?.isAutoAddMargin===true?true:previous?.isAutoAddMargin===false?false:null,

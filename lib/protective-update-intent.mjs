...
import { MAX_LOSS_ALGO_SHAPE } from './maxloss-order-shape.mjs';
...
export function buildProtectiveAlgoPlan({
  commandId,symbol,direction,quantity,triggerPrice,limitPrice=0,protectionKind='PROGRESSIVE',attempt=0
}={}){
  ...
  if(kind==='PROGRESSIVE'){
    ...
    const qty=positive(quantity,'QUANTITY_INVALID');
    params.type=MAX_LOSS_ALGO_SHAPE.type;
    params.timeInForce=MAX_LOSS_ALGO_SHAPE.timeInForce;
    params.quantity=String(qty);
    params.reduceOnly='true';
    params.priceMatch=MAX_LOSS_ALGO_SHAPE.priceMatch;
    params.clientAlgoId=deterministicClientOrderId({

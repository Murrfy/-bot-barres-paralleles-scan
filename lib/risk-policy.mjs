...
export function evaluateEntryRisk(input = {}) {
  ...
  const requestedMaxActive = input.maxActivePositions == null ? limits.maxActivePositions : num(input.maxActivePositions, NaN);
  const maxActivePositions = Number.isInteger(requestedMaxActive) &&
    requestedMaxActive >= 1 && requestedMaxActive <= limits.maxActivePositions
      ? requestedMaxActive
      : limits.maxActivePositions;
  if (input.maxActivePositions != null && maxActivePositions !== requestedMaxActive) reasons.push('MAX_ACTIVE_CONFIG_INVALID');
  ...
  if (!symbolConfig) {
    ...
    if (symbolConfig.isAutoAddMargin === true || String(symbolConfig.isAutoAddMargin || '').toLowerCase() === 'true') {
      reasons.push('AUTO_ADD_MARGIN_ENABLED');
    }

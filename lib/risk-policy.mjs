...
export function evaluateEntryRisk(input = {}) {
  ...
  if (activeCount >= limits.maxActivePositions) reasons.push('MAX_ACTIVE_POSITIONS_REACHED');
  if (positions.some(p => String(p?.symbol || '').toUpperCase() === symbol && Math.abs(num(p?.positionAmt)) > 0)) {
    reasons.push('SYMBOL_POSITION_ALREADY_OPEN');
  }
  if (standardOrders.some(o => String(o?.symbol || '').toUpperCase() === symbol) ||
      algoOrders.some(o => String(o?.symbol || '').toUpperCase() === symbol)) {
    reasons.push('SYMBOL_ORDER_ALREADY_OPEN');
  }
  ...
  if (referencePrice > 0 && tickSize > 0 && !stepAligned(referencePrice, tickSize)) reasons.push('PRICE_NOT_TICK_ALIGNED');
  if (referencePrice > 0 && minPrice > 0 && referencePrice < minPrice) reasons.push('PRICE_BELOW_EXCHANGE_MIN');
  if (referencePrice > 0 && maxPrice > 0 && referencePrice > maxPrice) reasons.push('PRICE_ABOVE_EXCHANGE_MAX');
  ...
  const bracket = bracketForNotional(bracketInfo, notional);
  if (!bracket) {

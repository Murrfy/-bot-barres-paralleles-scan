...
function response(){return{headers:{},setHeader(k,v){this.headers[k]=String(v)},status(n){this.code=n;return this},json(body){this.body=body;return body}}}
function request(){
  return {
    method:'POST',
    headers:{
...
function runtimeFixtures(){
  const data={executionMode:'REAL',userStream:{connected:true,ready:true,failClosed:false,needsReconciliation:false}};
  return {
    runtime:JSON.stringify({updatedAt:Date.now(),masterDeviceId:'master-1',data}),
    report:JSON.stringify({
      version:2,status:'CLEAN_REAL',failClosed:false,reasons:[],observedAt:Date.now(),
      runtimeDataHash:sha256(stableStringify(data)),
    }),
    arm:JSON.stringify({version:1,masterDeviceId:'master-1',deploymentSha:'sha-prod'}),
  };
}
function harness({permission=safePermission(),permissionHttpStatus=200}={}){
  const original=globalThis.fetch;
  const fx=runtimeFixtures();
  let futuresCalls=0,permissionCalls=0;
  globalThis.fetch=async(url,init={})=>{
    ...
    if(u.origin==='https://api.binance.com'){
      ...
      throw new Error('Unexpected permission path: '+u.pathname);
    }
  ...
  return{get futuresCalls(){return futuresCalls},get permissionCalls(){return permissionCalls},restore(){globalThis.fetch=original}};
}

test('unsafe trading-key permissions block real entry before Futures preflight',async()=>{

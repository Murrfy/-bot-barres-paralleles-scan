...
process.env.BINANCE_API_SECRET='secret-test';
...
function harness({permission=safePermission(),permissionHttpStatus=200}={}){
  ...
  globalThis.fetch=async(url,init={})=>{
    if(url==='https://redis.test'){
      ...
      if(c[0]==='GET'){
        if(String(c[1]).includes(':device:')){
          ...
          result=fx.runtime;
        }else if(c[1]==='zenith:v1:reconcile:last'){
          result=fx.report;
        }else if(c[1]==='zenith:v1:safety:real-execution-armed'){
          result=fx.arm;
        }else if(c[1]==='zenith:v1:master-mode'){
          result='RUNNING';
        }else if(c[1]==='zenith:v1:safety:emergency-stop'){
          result='0';
        }
    ...
    if(u.origin==='https://api.binance.com'){
      ...
      throw new Error('Unexpected Binance API permission path: '+u.pathname);
    }
...
test('unsafe Binance API permissions block a new real entry before Futures preflight',async()=>{
...
test('unavailable Binance permission check fails closed before Futures preflight',async()=>{

...
test('legacy Bearer migration rotates the token and normal Bearer auth is then rejected',async()=>{
  ...
  const record={deviceId:'iphone-12345678',role:'controller',deviceName:'iPhone',createdAt:Date.now()-1000,lastSeenAt:Date.now()-1000};
  ...
  try{
    ...
    await handler({method:'GET',query:{action:'whoami'},headers:{authorization:'Bearer legacy-token'}},bearerOnly);

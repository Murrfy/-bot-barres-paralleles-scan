...
if (!deviceSessionSource.includes('allowBearer = false') ||
    deviceSessionSource.includes('if (bearerToken(req)) return true') ||
    !sync.includes("action === 'whoami' && req.method === 'GET'") ||
    !sync.includes("{ allowBearer: true }") ||
    (sync.match(/allowBearer:\s*true/g) || []).length !== 1) {
  fail('legacy Bearer must be migration-only on whoami; all mutations and normal API auth must be cookie-only');
}

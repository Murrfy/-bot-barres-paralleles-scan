...
test('requested +7.80 protected floor rounds on the protective side, never below +7.80',()=>{
  const level=buildProgressiveProtectionLevel({
...
test('MAX-LOSS tick rounding never exceeds requested loss between ticks',()=>{
  const levels=buildRealProtectionLevels({
    position:{symbol:'BTCUSDT',positionSide:'BOTH',positionAmt:'0.7',entryPrice:'100'},
    targetProfitUsd:8,
    maxLossUsd:7.8,
...
test('a jump can select protection level 100 without truncation',()=>{

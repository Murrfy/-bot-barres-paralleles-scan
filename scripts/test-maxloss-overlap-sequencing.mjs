...
test('MAX-LOSS replacement confirms and publishes new stop before canceling old without interim full reconcile',()=>{
  ...
  assert.match(html,/if\(deferReconcile\)\{[\s\S]*await publishMasterStreamState\(\);[\s\S]*return clientId;[\s\S]*\}[\s\S]*const reconciled=await reconcileMasterUserStream\(\)/);
...
test('old MAX-LOSS cancellation still performs reconciliation after overlap is removed',()=>{
  ...
  assert.match(cancelBlock,/const reconciled=await reconcileMasterUserStream\(\)/);
});

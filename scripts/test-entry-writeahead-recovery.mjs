...
test('write-ahead recovery refuses any extra reconciliation mismatch or identity drift',()=>{
  ...
  assert.deepEqual(pendingEntryWriteAheadRecoveryTargets(report({closePosition:true})),[]);
  assert.deepEqual(pendingEntryWriteAheadRecoveryTargets(report({timeInForce:'GTC'})),[]);
  assert.deepEqual(pendingEntryWriteAheadRecoveryTargets(report({priceMatch:'OPPONENT_5'})),[]);
  assert.deepEqual(pendingEntryWriteAheadRecoveryTargets(report({origQty:'1'})),[]);

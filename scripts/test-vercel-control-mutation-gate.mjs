...
async function syncHandler(env,ref,suffix){
  if(env===undefined) delete process.env.VERCEL_ENV; else process.env.VERCEL_ENV=env;
  if(ref===undefined) delete process.env.VERCEL_GIT_COMMIT_REF; else process.env.VERCEL_GIT_COMMIT_REF=ref;
  process.env.UPSTASH_REDIS_REST_URL='https://redis.test';
  process.env.UPSTASH_REDIS_REST_TOKEN='test-only';
  return (await import('../api/zenith-sync.js?control-gate='+suffix)).default;
}
...
test('Vercel preview blocks Zenith POST before Redis/Binance',async()=>{
  ...
  try{
    const handler=await syncHandler('preview','feature-test','preview');
    const out=res();
...
test('production deployment from non-main blocks Zenith POST before external access',async()=>{
  let calls=0; globalThis.fetch=async()=>{calls++;throw new Error('external call forbidden')};
  try{
    const handler=await syncHandler('production','feature-test','prod-feature');
    const out=res();
...
test('local development and Vercel development pass only the central control mutation environment gate',async()=>{
  for(const [env,ref,suffix] of [[undefined,undefined,'local'],['development','feature-test','development']]){
    let calls=0; globalThis.fetch=async()=>{calls++;throw new Error('health action should not call external services')};
    try{
      const handler=await syncHandler(env,ref,suffix);
      const out=res();
...
test('production main passes the central control mutation environment gate',async()=>{
  let calls=0; globalThis.fetch=async()=>{calls++;throw new Error('health action should not call external services')};
  try{
    const handler=await syncHandler('production','main','prod-main');
    const out=res();
    await handler({method:'POST',query:{action:'health'},headers:headers(),body:{}},out);
    assert.notEqual(out.body?.code,'NON_PRODUCTION_CONTROL_MUTATION');
    assert.equal(out.code,404);
...
test('Vercel preview blocks reconciliation persistence before auth or external access',async()=>{
  ...
  try{
    ...
    process.env.UPSTASH_REDIS_REST_URL='https://redis.test';
    process.env.UPSTASH_REDIS_REST_TOKEN='test-only';
    const handler=(await import('../api/binance-reconcile.js?control-gate=preview')).default;
    const out=res();
